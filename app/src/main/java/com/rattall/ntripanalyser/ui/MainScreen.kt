            ...
            item {
                SectionCard(title = "RTCM Message Types") {
                    if (ui.messageSummaries.isEmpty()) {
                        Text("No RTCM message types received yet.", style = MaterialTheme.typography.bodyMedium)
                    } else {
                        ...
                        ui.messageSummaries.forEach { summary ->
                            ...
                            MessageSummaryRow(
                                summary = summary,
                                expanded = expanded,
                                onToggle = { expandedByType[summary.messageType] = !expanded }
                            )
                            if (expanded) {
...
private fun renderFieldValue(value: Any?): String {
    return when (value) {
        null -> "null"
        is List<*> -> if (value.isEmpty()) "[]" else "[${value.joinToString(", ") { renderNestedValue(it) }}]"
        is Map<*, *> -> {
            if (value.isEmpty()) {
                "{}"
            } else {
